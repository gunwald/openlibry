...
export async function getPublicBookDetail(
  ...
  return {
    ...
    maxAge: book.maxAge,
    relatedBooks,
    // Attached here rather than in the API route: the detail page reads this
    // function directly and never calls the route, so anything added there
    // alone never reaches the page.
    copies: await getCopySiblings(client, id),
  };
}
